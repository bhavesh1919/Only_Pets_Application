...
class DetailScreen extends StatefulWidget {
  ...
  const DetailScreen({super.key, required this.title, required this.image, required this.price});
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(
            child: Container(
              height: 250,
              width: 250,
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), color: Colors.grey[200], image: DecorationImage(image: AssetImage(widget.image), fit: BoxFit.contain)),
            ),
          ),
          const SizedBox(height: 20),
          Text(widget.title, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          const SizedBox(height: 6),
          Text("₹${widget.price.toStringAsFixed(2)}", style: const TextStyle(fontSize: 20, color: Colors.red, fontWeight: FontWeight.w500)),
          ...
          const Text("This is a premium pet product carefully selected to keep your pet healthy and happy.", style: TextStyle(color: Colors.black87, fontSize: 15)),
          const Spacer(),
          Container(
            ...
            child: ElevatedButton(
              ...
              onPressed: () {
                final index = cartItems.indexWhere((p) => p.title == widget.title);
                setState(() {
                  if (index != -1) {
                    cartItems[index].quantity += quantity;
                  } else {
                    cartItems.add(Product(title: widget.title, image: widget.image, price: widget.price, quantity: quantity));
                  }
                });

                Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const CartScreen()));
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${widget.title} added to cart successfully!')));
              },
